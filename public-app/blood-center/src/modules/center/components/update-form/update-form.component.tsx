...
export const UpdateCenterForm = () =>{
    ...
    <Flex margin='auto' justifyContent='center' width='100%' className="update-profile-form" border='1px solid lightgray' w={600} p={20}>
        <TemplateForm header={"Update Center"} buttonText={"Save"} onSubmit={handleSubmit}>
            <>
                <TemplateErrorInput
                ...
                <h2>Work Time</h2>
                <Grid templateColumns='repeat(1, 1fr)' templateRows='repeat(1, 1fr)' gap={5} width="100%">
                    {
                        // Object.keys(workTimes).map((key,index) =><p key={index}>{JSON.stringify(workTimes[key])}</p> )
                        Object.keys(workTimes).map((key,index) =>
                            <GridItem key={index}>
                                <SingleTimeDisplay time={workTimes[key]} onChangeFrom={(e)=>onChangeTimeFrom(e.target.value,key)}
                                 onChangeTo={(e)=>onChangeTimeTo(e.target.value,key)}/>
                            </GridItem>)
                    }
                </Grid>
            </>
